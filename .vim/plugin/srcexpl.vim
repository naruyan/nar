...
function! g:SrcExpl_PGo()

    let bufnum = winbufnr(g:SrcExpl_Window)
    if bufnum == -1
        let bufnum = bufwinnr(s:SrcExpl_pluginCaption)
        if bufnum == -1
            return 0
        endif
        let g:SrcExpl_Window = bufnum
    endif

    exe 'silent! ' . g:SrcExpl_Window . 'wincmd w'
...
function! g:SrcExpl_PNew(wincmd)
...
function! g:SrcExpl_PEdit(wincmd)

    if g:SrcExpl_PGo() == 0
       call g:SrcExpl_PNew(a:wincmd) 
    endif
...
function! g:SrcExpl_Pre()

    if winnr() == g:SrcExpl_Window
        return 1
    endif
...
function! g:SrcExpl_GoBack()
    ...
    if g:SrcExpl_Pre() || <SID>SrcExpl_AdaptPlugins()
        return -1
    endif
...
function! <SID>SrcExpl_ViewOneDef(fpath, excmd)
    ...
    if tagfiles()[0] ==# "tags" && a:fpath[0] == '.'
        call g:SrcExpl_PEdit(expand('%:p:h') . '/' . a:fpath)
    " Up to other directories
    else
        call g:SrcExpl_PEdit(a:fpath)
    endif
...
function! <SID>SrcExpl_OpenWin()
    ...
    let l:winnum = winbufnr(g:SrcExpl_Window)
